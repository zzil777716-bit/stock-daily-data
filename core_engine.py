...
if __name__ == "__main__":
    ...
    print("=" * 40)
    print("  홍군 퀀트 v7.4.2 Core 엔진 단독 구동 모드")
    print("=" * 40)
    
    db_dir = Path("database")
    db_dir.mkdir(exist_ok=True)
    db_path = db_dir / "quant_research_core.db"
    ...
    print(f"[*] 세션 ID 생성 완료: {session_id}")
    print("[*] Durable Database Writer 스레드 가동 중...")
    print("[*] 엔진이 정상 대기 상태입니다. 종료하려면 창을 닫거나 Ctrl+C를 누르세요.")
    
    try:
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        print("\n[*] 종료 신호 감지. 큐를 비우고 안전하게 종료합니다...")
        writer.stop()
        db_core.close()
        print("[*] 엔진이 안전하게 종료되었습니다.")